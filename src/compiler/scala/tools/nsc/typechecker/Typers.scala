      ...
      tree.tpe match {
        ...
        case _ =>
          ...
          if (tree.isType)
            ...
            if (settings.Xmacros.value) {
              val tree1 = expandMacro(tree)
              if (tree1.isErroneous) tree1 else typed(tree1, mode, pt)
            } else {
              errorTree(tree, tree.symbol.name + " is a macro, enable -Xmacros to proceed")
            }
    ...
    def typedArgs(args0: List[Tree], mode: Int, formals0: List[Type], adapted0: List[Type]): List[Tree] = {
      ...
      def loop(args: List[Tree], formals: List[Type], adapted: List[Type]): List[Tree] = {
        if (args.isEmpty || adapted.isEmpty) Nil
          ...
          val tree = formals.headOption match {
            // this is a very bad way to check for @untyped => doing this only because I need a quick & dirty solution
            case Some(AnnotatedType(List(AnnotationInfo(atp, _, _)), _, _)) if atp.typeSymbol.name.toString == "untyped" =>
              args.head
            case _ =>
              typedArg(args.head, mode, typedMode, adapted.head)
          }
        ...
        case mt @ MethodType(params, _) =>
          ...
          if (!sameLength(formals, args) ||   // wrong nb of arguments
            ...
            if (tparams.isEmpty) { // all type params are defined
              ...
              if ((args1 exists {arg => arg.tpe != null && arg.tpe.typeSymbol.isMagic}) && !fun0.symbol.isLift) {
                val typed = treeCopy.Apply(tree, fun, args1)
                typed setType MagicClass.tpe
                typed.fun setType MagicMethod.tpe
                return typed
...
//      if ((mode & TYPEmode) != 0) println("type: "+tree1+" has type "+tree1.tpe)
    ...
    def expandMacro(tree: Tree): Tree = try {
      // todo. uncomment this once the macro design pattern is established
//      macroExpand(tree) match {
//        case t: Tree => t
//        case t => errorTree(tree, "macros must return a compiler-specific tree; returned class is: " + t.getClass)
//      }
      val expanded = macroExpand(tree)
      if (expanded.isInstanceOf[Tree]) {
        expanded.asInstanceOf[Tree]
      } else {
        errorTree(tree, "macros must return a compiler-specific tree; returned class is: " + expanded.getClass)
      }
    } catch {
      case MacroExpandError(msg) => errorTree(tree, msg)
    }
