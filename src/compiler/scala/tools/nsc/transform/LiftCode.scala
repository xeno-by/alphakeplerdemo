    ...
    private def reifyTree(tree: Tree): Tree = tree match {
      ...
      case tt: TypeTree if (tt.tpe != null) =>
        if (definedInLiftedCode(tt.tpe)) {
          ...
          if (tt.original != null) reify(tt.original) else mirrorCall(nme.TypeTree)
        } else {
          var rtt = mirrorCall(nme.TypeTree, reifyType(tt.tpe))
          if (tt.original != null) {
            val setOriginal = Select(rtt, nme.setOriginal)
      ...
      case Apply(fun, args) if fun.symbol.isSplice =>
        ...
        val rmTree = Select(Ident(newTermName(nme.MIRROR_SHORT)), newTypeName("Tree"))
