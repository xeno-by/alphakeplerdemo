...
//      t
      val t = atPos(start) { Ident(newTypeName("Any")) }
      val untyped = Select(Select(Select(Ident(newTermName("scala")), newTermName("reflect")), newTermName("api")), newTypeName("untyped"))
      val annotation = Apply(Select(New(untyped), newTermName("<init>")), List())
      atPos(start) { Annotated(annotation, t) }
    }
