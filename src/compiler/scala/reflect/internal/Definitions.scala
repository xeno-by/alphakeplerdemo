...
trait Definitions extends reflect.api.StandardDefinitions {
  ...
  object definitions extends AbsDefinitions with ValueClassDefinitions {
    ...
    lazy val Code_ulift            = getMember(CodeModule, nme.ulift_)
    lazy val Code_splice           = getMember(CodeModule, nme.splice_)
    lazy val Code_usplice          = getMember(CodeModule, nme.usplice_)
    lazy val Code_typed            = getMember(CodeModule, nme.typed_)
    lazy val UcodeClass            = getClass(sn.Ucode)
    lazy val UcodeModule           = getModule(sn.Ucode)
    lazy val Ucode_lift            = getMember(UcodeModule, nme.lift_)
    lazy val Ucode_splice          = getMember(UcodeModule, nme.splice_)
    ...
    lazy val QuasiquoteClass           = getRequiredClass("scala.reflect.Quasiquote")
    lazy val QuasiquoteLiteralClass    = getRequiredClass("scala.reflect.QuasiquoteLiteral")
    lazy val QuasiquotePatternClass    = getRequiredClass("scala.reflect.QuasiquotePattern")
    lazy val StringContextClass        = getRequiredClass("scala.runtime.StringContext")
    lazy val StringContext_show        = getMember(StringContextClass, nme.show_)
    lazy val FormattedClass            = getRequiredClass("scala.runtime.Formatted")
